...
--
-- Temporary view structure for view `v_name_alder`
--
...
--
-- Temporary view structure for view `vlonerevision`
--

DROP TABLE IF EXISTS `vlonerevision`;
/*!50001 DROP VIEW IF EXISTS `vlonerevision`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8mb4;
/*!50001 CREATE VIEW `vlonerevision` AS SELECT 
 1 AS `akronym`,
 1 AS `fornamn`,
 1 AS `efternamn`,
 1 AS `prekomp`,
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`user`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `v_lonerevision` AS select `l`.`akronym` AS `akronym`,`l`.`fornamn` AS `fornamn`,`l`.`efternamn` AS `efternamn`,`p`.`lon` AS `pre`,`l`.`lon` AS `nu`,sum((`l`.`lon` - `p`.`lon`)) AS `diff`,round(((sum((`l`.`lon` - `p`.`lon`)) * 100.0) / `p`.`lon`),2) AS `proc`,if((round(((sum((`l`.`lon` - `p`.`lon`)) * 100.0) / `p`.`lon`),2) > 3),'ok','nok') AS `mini` from (`larare` `l` join `larare_pre` `p` on((`l`.`akronym` = `p`.`akronym`))) group by `l`.`akronym` order by `proc` desc */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
--
-- Final view structure for view `vlonerevision`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`user`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `vlonerevision` AS select `l`.`akronym` AS `akronym`,`l`.`fornamn` AS `fornamn`,`l`.`efternamn` AS `efternamn`,`p`.`kompetens` AS `prekomp`,`l`.`kompetens` AS `nukomp`,sum((`l`.`kompetens` - `p`.`kompetens`)) AS `diffkomp` from (`larare` `l` join `larare_pre` `p` on((`l`.`akronym` = `p`.`akronym`))) group by `l`.`akronym` order by `nukomp` desc,`diffkomp` desc */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2019-02-13 13:17:19
